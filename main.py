...
if opt.resume != None:
    model.load_state_dict(copyStateDict(torch.load(opt.resume)['state_dict']),False)
...
"""============================================================================"""
#################### LOSS SETUP ####################
#代码做了这样的修改，当使用multi_loss参量时输出双特征在结构的代码里面。先输出分类头，再输出patch头，添加了m_loss参量(一般使用softmax)，在后面的代码里面loss对接cls token，m_loss对接patch token，在后面可以修改alpha的值，现在是m-loss占0.9
#如果和原来一样只使用单loss，修改m_loss就可以，测试时的特征按alpha融合在metrics文件夹的init里面，不过无需针对修改，当输出双特征时自动做特征融合
#单loss如果是度量学习loss需要人为加lr衰减，softmax不需要
...
batchminer   = bmine.select(opt.batch_mining, opt)
criterion_m, to_optim = criteria.select(opt.m_loss, opt, to_optim, batchminer)
_ = criterion_m.to(opt.device)
...
if opt.multi_loss:
    setup_text = 'Objective:\t {} + {}'.format(opt.loss.upper(),opt.m_loss.upper())
else:
    setup_text = 'Objective:\t {}'.format(opt.m_loss.upper())
...
loss_args_cls  = {'batch':None, 'labels':None, 'batch_features':None, 'f_embed':None}
loss_args_patch  = {'batch':None, 'labels':None, 'batch_features':None, 'f_embed':None}
...
if opt.test:
    ...
    eval.evaluate(opt.dataset, LOG, metric_computer, [dataloaders['testing']],    model, opt, opt.evaltypes, opt.device, make_recall_plot = True, log_key='Test')
    exit()
...
for epoch in range(opt.n_epochs):
    ...
    for i,out in enumerate(data_iterator):
        ...
        if opt.multi_loss:
            embeds_cls, embeds_patch  = model(**model_args)
        # pdb.set_trace()
            if isinstance(embeds_cls, tuple): embeds_cls, (avg_features, features) = embeds_cls
            if isinstance(embeds_patch, tuple): embeds_patch, (avg_features, features) = embeds_patch
            ### Compute Loss
            ...
            # else:
            #     loss_args['f_embed']        = model.model.model.head
            # loss_args['batch_features'] = features
            # pdb.set_trace()
            loss_cls      = criterion(**loss_args_cls)
            loss_patch   = criterion_m(**loss_args_patch)
