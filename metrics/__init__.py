from metrics import e_recall, nmi, f1, mAP, mAP_c, mAP_1000, mAP_lim,e_precision
from metrics import dists, rho_spectrum
from metrics import c_recall, c_nmi, c_f1, c_mAP_c, c_mAP_1000, c_mAP_lim,c_precision
import numpy as np
import faiss
import torch
...
def select(metricname, opt):
    ...
    elif 'e_precision' in metricname:
        k = int(metricname.split('@')[-1])
        return e_precision.Metric(k)
    elif metricname=='nmi':
        return nmi.Metric()
    elif metricname=='mAP':
        return mAP.Metric()
    ...
    elif 'c_precision' in metricname:
        k = int(metricname.split('@')[-1])
        return c_precision.Metric(k)
...
class MetricComputer():
    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, **kwargs):
        ...
        with torch.no_grad():
            ...
            for idx,inp in enumerate(final_iter):
                ...
                if isinstance(out, tuple): 
                    ...
                    out = weight_alpha*out_cls + (1-weight_alpha)*out_patch
        ...
        for evaltype in evaltypes:
            ...
            # cluster(features,target_labels)
            start = time.time()
            ...
            if 'nearest_features_cosine' in self.requires:
                ...
                max_kval                   = np.max([int(x.split('@')[-1]) for x in self.metric_names if 'precision' in x])
